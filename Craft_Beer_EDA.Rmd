...
divisions<-divisions%>%
  mutate(division=case_when(states %in% c("CT","ME","MA","NH","RI","VT") ~ "New England",
                            states %in% c("NJ","NY","PA") ~ "Mid-Atlantic",
                            states %in% c("WV","MD","DE","DC","VA","NC","SC","GA","FL") ~ "South Atlantic",
                            states %in% c("KY","TN","MS","AL") ~ "East South Central",
                            states %in% c("MI","OH","IN","IL","WI") ~ "East North Central",
                            states %in% c("MN","IA","MO","ND","SD","NE","KS") ~ "West North Central",
                            states %in% c("OK","AR","LA","TX") ~ "West South Central",
                            states %in% c("CO","NM","WY","MT","ID","UT","AZ","NV") ~ "Mountain",
                            states %in% c("WA","OR","CA","AK","HI") ~ "Pacific",
                            TRUE ~ "OTHER"))
```
...
```{r corr_test}
ggplot(data=joineddata,aes(x=abv,y=ibu,color=state))+
  geom_point(alpha=.5)
